...
int DEFAULT_GLOBAL_TEST_RUN_DEADLINE_MS = 90000;
int DEFAULT_GLOBAL_TEST_RUN_WARNLINE_MS = 2000;
...
public:
    ...
    void SetTestTime(long int elapsed_time_ns){
        elapsed_time_ns_ = elapsed_time_ns;
    }

    long int GetTestTime() const{ return elapsed_time_ns_; }
...
public:
    ...
    void SetTestTime(size_t test_id, long int elapsed_time_ns){
        VerifyTestId(test_id);
        test_list_[test_id].SetTestTime(elapsed_time_ns);
    }

    long int GetTestTime(size_t test_id) const{
        VerifyTestId(test_id);
...
static void OnTestIterationStartPrint(const std::vector<TestCase>& testcase_list, size_t iteration,int iteration_count){
...
static void OnTestIterationEndPrint(const std::vector<TestCase>& testcase_list,size_t /*iteration*/,long int elapsed_time_ns){
    std::vector<std::string> fail_test_name_list;
    std::vector<std::pair<std::string, long int>> timeout_test_list;

    // For tests run exceed warnline but not timeout.
    std::vector<std::tuple<std::string, long int, int>> slow_test_list;
    size_t testcase_count = testcase_list.size();
    size_t test_count = 0;
...
void OnTestIterationEndXmlPrint(const std::string& xml_output_filename,const std::vector<TestCase>& testcase_list,time_t epoch_iteration_start_time,long int elapsed_time_ns){
    ...
    std::vector<long int> elapsed_time_list(testcase_list.size(), 0);
...
struct ChildProcInfo{
    pid_t pid;
    long int start_time_ns;
    long int end_time_ns;
    long int
...
static ChildProcInfo RunChildProcess(const std::string& test_name,int testcase_id, int test_id, int argc,char** argv){
    int pipefd[2];
    if (pipe(pipefd) == -1) {
...
static void HandleSignals(std::vector<TestCase>& testcase_list,std::vector<ChildProcInfo>& child_proc_list){
    if (sigquit_flag) {
        sigquit_flag = false;
        ...
        for (const auto& child_proc : child_proc_list) {
            if (child_proc.pid != 0) {
                std::string test_name =
                    ...
                    child_proc.test_id);
                long int current_time_ns = NanoTime();
                long int run_time_ms =
...
static bool CheckChildProcExit(pid_t exit_pid, int exit_status,std::vector<ChildProcInfo>& child_proc_list){
...
static void ReadChildProcOutput(std::vector<TestCase>& testcase_list,std::vector<ChildProcInfo>& child_proc_list){
    for (const auto& child_proc : child_proc_list) {
...
static void WaitChildProcs(std::vector<TestCase>& testcase_list,std::vector<ChildProcInfo>& child_proc_list){
...
static bool RunTestInSeparateProc(int argc, char** argv,std::vector<TestCase>& testcase_list,int iteration_count, size_t job_count,const std::string& xml_output_filename){
    ...
    for (size_t iteration = 1;
        ...
        ++iteration) {
            ...
            long int iteration_start_time_ns = NanoTime();
            ...
            long int elapsed_time_ns = NanoTime() - iteration_start_time_ns;
